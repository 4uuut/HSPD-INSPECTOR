...
/**
 * Auto-generate a secure, random 4-digit numeric PIN for Terminal MDT login.
 * Ensures the PIN is unique and not already assigned to any officer in the roster.
 */
export function generateRandomMdtPin(existingOfficers: OfficerAccount[] = []): string {
  ...
  // Try generating unique 4-digit PINs (1000 - 9999)
  for (let attempt = 0; attempt < 500; attempt++) {
    const candidate = Math.floor(1000 + Math.random() * 9000).toString();
    if (
      !usedPins.has(candidate) &&
      candidate !== '1234' &&
      candidate !== '1111' &&
      candidate !== '0000' &&
      candidate !== '9999' &&
  ...
  // Fallback 6-digit random PIN if space is saturated
...
export const RosterManagement: React.FC<Props> = ({
  ...
  const [addPin, setAddPin] = useState<string>(() => generateRandomMdtPin());
  ...
  const handleOpenAddModal = () => {
    ...
    const autoGeneratedPin = generateRandomMdtPin(roster);
    setAddPin(autoGeneratedPin);
    setShowAddPin(true);
    setAddDiscordTag('');
    setAddPhone('');
  ...
  const handleRegenerateAddPin = () => {
    const freshPin = generateRandomMdtPin(roster);
    setAddPin(freshPin);
    setShowAddPin(true);
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-3 sm:p-4 backdrop-blur-xs font-mono text-xs animate-in fade-in duration-150">
          <div className="bg-[#161B22] border border-amber-500/80 rounded-xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <form onSubmit={(e) => handleSaveNewOfficer(e, false)} className="p-4 sm:p-5 space-y-3.5 overflow-y-auto flex-1">
              ...
              {/* PIN Login Awal (Auto-Generated Unik & Acak) */}
              <div className="space-y-1.5 p-3 bg-amber-950/20 border border-amber-500/40 rounded-lg">
                <div className="flex items-center justify-between flex-wrap gap-1">
                  <label className="text-[10px] font-bold text-amber-300 uppercase flex items-center gap-1.5">
                    ...
                    <span className="px-1.5 py-0.5 bg-amber-500/20 border border-amber-500/40 rounded text-[9px] font-mono text-amber-300 flex items-center gap-1">
                      <span>🎲</span>
                      <span>Auto-Generated Unik</span>
                    </span>
                  </label>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={handleRegenerateAddPin}
                      className="px-2 py-0.5 bg-amber-600/30 hover:bg-amber-600/50 border border-amber-500/60 rounded text-[10px] text-amber-200 font-mono font-bold flex items-center gap-1 transition cursor-pointer active:scale-95 shadow-sm"
                      title="Generate ulang PIN acak baru yang belum dipakai"
                    >
                      <RefreshCw className="w-2.5 h-2.5" />
                      <span>Acak Ulang PIN</span>
                    </button>
                    <button
                      type="button"
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-3 pr-24 py-2 bg-[#0D1117] border border-amber-500/70 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 rounded-lg text-sm text-amber-200 outline-none font-mono font-bold tracking-wider"
                    required
                  />
                  <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-amber-400/80 font-mono">
                    {addPin ? `${addPin.length} Digit` : 'Kosong'}
                  </div>
                </div>
                <div className="text-[9.5px] text-gray-400 flex items-center gap-1">
                  <span>⚡</span>
                  <span>PIN ter-generate secara otomatis & acak khusus untuk akun ini agar tidak sama antar personel.</span>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                  ...
                  <div className="lg:col-span-5 space-y-3 lg:sticky lg:top-0">
                    ...
                    <div className="bg-[#1E1F22] border border-[#313338] rounded-xl p-3 shadow-xl">
                        ...
                        <div className="space-y-1.5 pt-0.5 font-mono text-[11px]">
                          ...
                          <div>
                            <div className="text-[9.5px] font-bold text-sky-400 uppercase flex items-center gap-1">
                              <span>🌐</span>
                              <span>Akses Terminal MDT Web</span>
                            </div>
                            <div className="text-sky-200 text-[10.5px] font-sans mt-0.5">
                              👉 <span className="text-sky-300 underline font-mono text-[10px]">https://mdc-hspd-inspector.vercel.app/</span>
                            </div>
                          </div>
                      ...
                      {/* Discord Link Button Mockup (Interactive Action Row) */}
                      <div className="mt-2.5">
                        <a
                          href="https://mdc-hspd-inspector.vercel.app/"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-2 px-3 bg-[#4E5058] hover:bg-[#6D6F78] text-white rounded-md text-xs font-semibold flex items-center justify-center gap-2 transition no-underline shadow cursor-pointer"
                        >
                          <span>🌐</span>
                          <span>Akses Terminal MDT Web</span>
