...
import { 
  ...
  Edit3, RotateCcw, PlusCircle, Save, FileSpreadsheet,
...
import { PasalExcelImportModal } from './PasalExcelImportModal';
import { 
...
export const PasalCalculator: React.FC<Props> = ({ onSaveRecord, currentOfficer }) => {
  ...
  const handleApplyExcelImport = (newPasalList: PasalItem[], mode: 'replace' | 'merge') => {
    let finalList: PasalItem[] = [];
    if (mode === 'replace') {
      finalList = newPasalList;
    } else {
      const map = new Map<string, PasalItem>();
      pasalList.forEach(p => map.set(p.code.toUpperCase(), p));
      newPasalList.forEach(p => map.set(p.code.toUpperCase(), p));
    ...
    setPasalNotice({
      type: 'success',
      text: `✅ Sukses! ${newPasalList.length} pasal KUHP & SOP berhasil diperbarui dari file Excel (${mode === 'replace' ? 'Ganti Total' : 'Gabung Data'}).`
    });
    setTimeout(() => setPasalNotice(null), 6000);
  };
  ...
  return (
    <div id="pasal-calculator-root" className="space-y-4">
      ...
      {calcViewMode === 'citizen' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* Import Excel / CSV Button (Khusus Atasan) */}
            <button
              id="btn-import-excel-kuhp"
              type="button"
              onClick={handleRequestImport}
              className={`px-2.5 py-1 rounded text-[11px] font-bold flex items-center gap-1.5 transition shadow-sm ${
                canManagePasal
                  ? 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white shadow-emerald-950/40 cursor-pointer'
                  : 'bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700'
              }`}
              title={canManagePasal ? "Upload file Excel (.xlsx / .csv) untuk mengubah / mengganti seluruh pasal KUHP & SOP secara otomatis" : "Khusus Atasan / Personel berizin (Klik untuk otorisasi)"}
            >
              {canManagePasal ? <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-200" /> : <Lock className="w-3 h-3 text-amber-400" />}
      ...
      {/* EXCEL IMPORT / BULK UPDATE MODAL */}
      <PasalExcelImportModal
        isOpen={showExcelImportModal}
        onClose={() => setShowExcelImportModal(false)}
        currentPasalList={pasalList}
        onApplyImport={handleApplyExcelImport}
        isAtasan={canManagePasal}
      />
