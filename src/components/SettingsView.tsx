...
export const SettingsView: React.FC<Props> = ({
  ...
  // Registration Panel to Discord Channel State
  const [regChannelId, setRegChannelId] = useState(botConfig.registrationChannelId || '');
  const [regTitle, setRegTitle] = useState(botConfig.registrationEmbedTitle || 'UCP Panel High State');
  const [regDesc, setRegDesc] = useState(botConfig.registrationEmbedDesc || DEFAULT_REGISTRATION_PANEL_DESC);
  const [regColor, setRegColor] = useState(botConfig.registrationEmbedColor || '#00A8FF');
  const [regThumbnail, setRegThumbnail] = useState(botConfig.registrationThumbnail || 'https://cdn-icons-png.flaticon.com/512/1022/1022382.png');
  const [regFooter, setRegFooter] = useState(botConfig.registrationFooter || 'Bot High State');
  const [regRegisterUrl, setRegRegisterUrl] = useState(botConfig.registrationRegisterUrl || '');
  const [availableChannels, setAvailableChannels] = useState<Array<{ id: string; name: string; guildName: string; type: number }>>([]);
  const [isLoadingChannels, setIsLoadingChannels] = useState(false);
  const [isSendingRegPanel, setIsSendingRegPanel] = useState(false);
  const [regPanelNotice, setRegPanelNotice] = useState<{ success: boolean; message: string } | null>(null);

  // Sync internal state when botConfig changes
  useEffect(() => {
    if (botConfig.registrationChannelId && !regChannelId) setRegChannelId(botConfig.registrationChannelId);
    if (botConfig.registrationEmbedTitle && regTitle === 'UCP Panel High State') setRegTitle(botConfig.registrationEmbedTitle);
    if (botConfig.registrationEmbedDesc && regDesc === DEFAULT_REGISTRATION_PANEL_DESC) setRegDesc(botConfig.registrationEmbedDesc);
    if (botConfig.registrationEmbedColor) setRegColor(botConfig.registrationEmbedColor);
    if (botConfig.registrationThumbnail) setRegThumbnail(botConfig.registrationThumbnail);
    if (botConfig.registrationFooter) setRegFooter(botConfig.registrationFooter);
    if (botConfig.registrationRegisterUrl) setRegRegisterUrl(botConfig.registrationRegisterUrl);
  }, [botConfig]);

  const handleFetchChannels = async () => {
    setIsLoadingChannels(true);
    try {
      const chs = await fetchDiscordChannels(botConfig.botToken);
      setAvailableChannels(chs);
      if (chs.length === 0) {
        setRegPanelNotice({
          success: false,
          message: 'Tidak ada channel teks yang ditemukan. Pastikan Bot Token valid dan Bot sudah diundang ke server Discord Anda.'
        });
      } else {
        setRegPanelNotice({
          success: true,
          message: `Berhasil mendeteksi ${chs.length} channel dari server Discord tempat Bot berada.`
        });
      }
      setTimeout(() => setRegPanelNotice(null), 4000);
    } catch (err: any) {
      setRegPanelNotice({
        success: false,
        message: `Gagal memuat channel: ${err?.message || err}`
      });
    } finally {
  ...
  const handleSaveRegPanelSettings = () => {
    const updated: DiscordBotConfig = {
      ...botConfig,
      registrationChannelId: regChannelId.trim(),
      registrationEmbedTitle: regTitle.trim(),
      registrationEmbedDesc: regDesc.trim(),
      registrationEmbedColor: regColor.trim(),
      registrationThumbnail: regThumbnail.trim(),
      registrationFooter: regFooter.trim(),
      registrationRegisterUrl: regRegisterUrl.trim()
    };
    setBotConfig(updated);
    saveDiscordBotConfig(updated);
    setRegPanelNotice({
      success: true,
      message: '✅ Pengaturan Channel & Format Panel Registrasi berhasil disimpan!'
    });
    setTimeout(() => setRegPanelNotice(null), 5000);
  };

  const handleSendRegistrationPanelNow = async () => {
    if (!regChannelId.trim()) {
      setRegPanelNotice({
        success: false,
        message: '⚠️ Masukkan atau pilih ID Channel Discord tujuan terlebih dahulu!'
      });
      return;
    }
    setIsSendingRegPanel(true);
    setRegPanelNotice(null);
    try {
      const res = await sendRegistrationPanelToDiscord({
        channelId: regChannelId.trim(),
        embedTitle: regTitle.trim(),
        embedDescription: regDesc.trim(),
        embedColor: regColor.trim(),
        thumbnailUrl: regThumbnail.trim(),
        footerText: regFooter.trim(),
        registerUrl: regRegisterUrl.trim(),
        botName: botConfig.botName,
        botToken: botConfig.botToken
      });
      setRegPanelNotice(res);
      setTimeout(() => setRegPanelNotice(null), 8000);
    } catch (e: any) {
      setRegPanelNotice({
        success: false,
        message: `Gagal mengirim panel registrasi: ${e?.message || e}`
      });
    } finally {
      setIsSendingRegPanel(false);
    }
  };
  ...
  const handleSendTestDm = async () => {
    if (!testDmUserId.trim()) {
      setBotNotice({
        success: false,
        message: 'Masukkan Username atau User ID target untuk menguji kiriman PM.'
      });
      return;
    }
    ...
    try {
      const res = await sendOfficerDirectMessageViaBot({
        ...
        discordUsername: testDmUserId.trim(),
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="bg-[#121620] border border-gray-800 rounded-xl p-4 sm:p-5 shadow-xl space-y-4">
        ...
        <div className="bg-[#0B0D12] border border-amber-900/60 rounded-xl p-2.5 sm:p-3 shadow-inner">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5 overflow-x-auto no-scrollbar py-1">
            ...
            {/* BUTTON 8: 📢 👑 PANEL REGISTRASI CHANNEL DISCORD */}
            <button
              id="btn-settings-registration-channel"
              type="button"
              onClick={() => {
                setActiveCategory('bot_pm');
                setTimeout(() => {
                  document.getElementById('section-registration-channel-panel')?.scrollIntoView({ behavior: 'smooth' });
                }, 100);
              }}
              className="px-3.5 py-2.5 bg-gradient-to-r from-indigo-950 to-purple-950 hover:from-indigo-900 hover:to-purple-900 text-indigo-300 border border-indigo-500/80 hover:border-indigo-400 rounded-lg text-xs font-bold font-mono transition flex items-center gap-2 shrink-0 shadow-md shadow-indigo-950/40 group active:scale-95 cursor-pointer"
              title="Pengaturan Pengiriman Pesan Panel Registrasi MDT ke Channel Discord Tertentu"
            >
              <Bot className="w-4 h-4 text-indigo-400 group-hover:scale-110 transition" />
              <div className="flex flex-col items-start text-left leading-tight">
                <span className="text-[10px] text-indigo-400/90 font-normal flex items-center gap-1">👑 PANEL REGISTER</span>
                <span className="font-bold text-indigo-200">CHANNEL DISCORD</span>
              </div>
            </button>
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="p-3.5 bg-[#0D1117] border border-gray-800 rounded-xl space-y-2">
                <label className="block text-xs font-bold text-gray-200 uppercase font-mono flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-emerald-400 font-semibold">Username atau User ID</span>
                </label>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    ...
                    placeholder="Masukkan Username Discord (contoh: aguy atau @aguy, atau ID)"
      ...
      {/* ========================================================================= */}
      {/* SECTION: PANEL REGISTRASI ANGGOTA BARU & LOGIN MDT KE CHANNEL DISCORD     */}
      {/* ========================================================================= */}
      {(activeCategory === 'all' || activeCategory === 'bot_pm' || activeCategory === 'security_auth' || activeCategory === 'roster_mgmt') && (
        <div id="section-registration-channel-panel" className="bg-[#121620] border border-indigo-700/80 rounded-xl p-4 sm:p-5 shadow-2xl space-y-5">
          {/* Header */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-gray-800">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-indigo-950 border border-indigo-500/70 flex items-center justify-center text-indigo-400 shrink-0">
                <Bot className="w-5 h-5" />
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-base font-bold text-white flex items-center gap-2 font-mono">
                    <span>PANEL REGISTRASI ANGGOTA BARU KE CHANNEL DISCORD</span>
                  </h3>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded border border-indigo-500/60 bg-indigo-950/80 text-indigo-300 font-bold">
                    INTERAKTIF (REGISTER, RESEND, LUPA PW, ROLE)
                  </span>
                </div>
                <p className="text-xs text-gray-400 font-mono">
                  Bot mengirim pesan panel registrasi interaktif ke channel Discord yang ditentukan (bisa diatur channel tujuan, teks, warna & link).
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                type="button"
                onClick={handleSaveRegPanelSettings}
                className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold font-mono transition flex items-center gap-1.5 shadow-md shadow-indigo-950 cursor-pointer"
              >
                <Save className="w-3.5 h-3.5" />
                <span>Simpan Pengaturan Panel</span>
              </button>

              <button
                type="button"
                onClick={handleSendRegistrationPanelNow}
                disabled={isSendingRegPanel || !regChannelId.trim()}
                className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-lg text-xs font-bold font-mono transition flex items-center gap-1.5 shadow-md shadow-emerald-950 cursor-pointer active:scale-95"
              >
                {isSendingRegPanel ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                <span>📢 Kirim Panel ke Channel Sekarang</span>
              </button>
            </div>
          </div>
          ...
          {regPanelNotice && (
            <div className={`p-3 rounded-lg border text-xs flex items-center gap-2 font-mono animate-in fade-in ${
              ...
              <span>{regPanelNotice.message}</span>
            </div>
          )}

          {/* 2-Column Responsive: Form on Left, Live Discord Channel Mockup on Right */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            {/* Left Inputs */}
            <div className="lg:col-span-7 space-y-4">
              {/* Channel ID Input with Auto-Fetch channels */}
              <div className="space-y-1.5 p-3.5 bg-[#0D1117] border border-indigo-900/60 rounded-xl">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-indigo-300 uppercase font-mono flex items-center gap-1.5">
                    <span>ID CHANNEL DISCORD TUJUAN:</span>
                  ...
                  >
                    ...
                    <span>Ambil List Channel Server</span>
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    value={regChannelId}
                    onChange={(e) => setRegChannelId(e.target.value)}
                    placeholder="Masukkan ID Channel Discord (contoh: 1234567890123456789)"
                    className="flex-1 px-3 py-2 bg-[#161B22] border border-gray-700 focus:border-indigo-500 rounded-lg text-xs text-indigo-200 font-mono outline-none"
                  />
                  {availableChannels.length > 0 && (
                    <select
                      onChange={(e) => {
                        if (e.target.value) setRegChannelId(e.target.value);
                      }}
                      className="px-2.5 py-2 bg-[#161B22] border border-gray-700 rounded-lg text-xs text-gray-200 font-mono outline-none max-w-[180px]"
                    >
                ...
                <p className="text-[10.5px] text-gray-400 font-mono">
                  💡 Cara dapat Channel ID: Di Discord, klik kanan nama channel teks (misal <code>#daftar-ucp-mdt</code>) &gt; <strong>Copy Channel ID</strong> (pastikan Developer Mode aktif).
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="UCP Panel High State"
                    className="w-full px-3 py-2 bg-[#0D1117] border border-gray-700 focus:border-indigo-500 rounded-lg text-xs text-gray-100 font-mono outline-none"
                  />
                </div>
              ...
              {/* Description Body (Matches screenshot from image.png) */}
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-gray-200 uppercase font-mono">
                    Isi Pesan Deskripsi (Format Kotak Teks Sesuai Gambar):
                  </label>
                  <button
                    type="button"
                    onClick={() => setRegDesc(DEFAULT_REGISTRATION_PANEL_DESC)}
                    className="text-[10px] text-sky-400 hover:underline font-mono"
                  >
                    Reset Teks Sesuai Gambar
                  </button>
                </div>
                <textarea
                  value={regDesc}
                  ...
                  rows={8}
                  className="w-full px-3 py-2 bg-[#0D1117] border border-gray-700 focus:border-indigo-500 rounded-lg text-xs text-gray-100 font-sans leading-relaxed outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-gray-200 uppercase font-mono">
                    URL Logo Thumbnail (Kanan Atas Embed):
                  </label>
                  <input
                    type="text"
                    value={regThumbnail}
                    onChange={(e) => setRegThumbnail(e.target.value)}
                    placeholder="https://..."
                    className="w-full px-3 py-2 bg-[#0D1117] border border-gray-700 focus:border-indigo-500 rounded-lg text-xs text-gray-100 font-mono outline-none"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={regFooter}
                    onChange={(e) => setRegFooter(e.target.value)}
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-gray-200 uppercase font-mono">
                  Link Tombol "Register" (Opsional URL MDT Web):
                </label>
                <input
                  type="text"
                  value={regRegisterUrl}
                  onChange={(e) => setRegRegisterUrl(e.target.value)}
                  placeholder="Kosongkan untuk tombol interaktif Bot, atau isi https://domain-anda/register"
                  className="w-full px-3 py-2 bg-[#0D1117] border border-gray-700 focus:border-indigo-500 rounded-lg text-xs text-gray-100 font-mono outline-none"
                />
              </div>
            </div>

            {/* Right Column: Exact Discord Message Replica of the image */}
            <div className="lg:col-span-5 flex flex-col space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-gray-300 font-mono flex items-center gap-1.5">
                  <Eye className="w-3.5 h-3.5 text-indigo-400" />
                  <span>Pratinjau Pesan Channel (Persis Gambar):</span>
                </span>
                <span className="text-[10px] font-mono text-emerald-400">#channel-preview</span>
              </div>

              {/* Discord Chat Box */}
              <div className="bg-[#313338] border border-gray-700/60 rounded-xl p-4 text-gray-200 font-sans shadow-2xl space-y-3 flex-1">
                {/* Bot Profile */}
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <img
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-sm text-white">{botConfig.botName || 'High State Roleplay'}</span>
                      <span className="text-[10px] bg-[#5865F2] text-white px-1.5 py-0.2 rounded font-bold">
                        BOT
                      </span>
                    </div>
                ...
                {/* The Embed Box with 4-button action row */}
                <div
                  className="bg-[#2B2D31] rounded-lg p-3.5 border-l-4 space-y-3"
                  style={{ borderLeftColor: regColor || '#00A8FF' }}
                >
                  {/* Author / Title with thumbnail on right */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="text-[11px] text-gray-400 font-semibold">High State Roleplay</div>
                      <div className="font-bold text-sm text-white">
                        {regTitle || 'UCP Panel High State'}
                      </div>
                    </div>
                    {regThumbnail && (
                      <img
                        src={regThumbnail}
                        alt="Thumbnail"
                        className="w-14 h-14 rounded-lg object-contain bg-black/40 shrink-0"
                        referrerPolicy="no-referrer"
                      />
                    )}
                  </div>
                  ...
                  {/* Description body */}
                  <div className="text-xs text-[#DBDEE1] whitespace-pre-line leading-relaxed font-sans max-h-72 overflow-y-auto pr-1">
                    {regDesc}
                  </div>
                  ...
                  <div className="pt-2 border-t border-gray-700/40 flex items-center gap-1.5 text-[10px] text-[#949BA4]">
                    <img
                      ...
                      referrerPolicy="no-referrer"
                    />
                    <span>{regFooter || 'Bot High State'} • Hari ini pukul 14:32</span>
                  </div>
                </div>

                {/* 4 Interactive Buttons exactly like in the picture */}
                <div className="pt-1 flex flex-wrap gap-2">
                  <button
                    type="button"
                    className="px-3.5 py-1.5 bg-[#248046] hover:bg-[#1a6334] text-white rounded font-medium text-xs flex items-center gap-1.5 shadow cursor-pointer"
                  >
                    <span>📄</span>
                    <span>Register</span>
                  </button>
                  ...
                  <button
                    type="button"
                    className="px-3.5 py-1.5 bg-[#5865F2] hover:bg-[#4752C4] text-white rounded font-medium text-xs flex items-center gap-1.5 shadow cursor-pointer"
                  >
                    <span>♻️</span>
                  ...
                  <button
                    type="button"
                    className="px-3.5 py-1.5 bg-[#DA373C] hover:bg-[#A1282C] text-white rounded font-medium text-xs flex items-center gap-1.5 shadow cursor-pointer"
                  >
                    <span>🚨</span>
                  ...
                  <button
                    type="button"
                    className="px-3.5 py-1.5 bg-[#4E5058] hover:bg-[#6D6F78] text-white rounded font-medium text-xs flex items-center gap-1.5 shadow cursor-pointer"
                  >
                    <span>⚙️</span>
                    <span>Take Role</span>
                  </button>
